...
//Se o jogador estiver no alcance do ataque do arqueiro
if(point_distance(x,y,obj_player.x,obj_player.y) < attack_range){
	if (!collision_line(x, y, obj_player.x, obj_player.y, obj_wall, false, true)) {
        if(alarm[5] == -1){
			...
			scr_change_combat_sprite(self, [sprite_attack_up, sprite_attack_right, sprite_attack_down, sprite_attack_left], "bow", true);
